    ...
    with tf.variable_scope("synthesis"):
        with tf.variable_scope("layer_0"):
            layer = sigconv.SignalConv2D(
                ...
                padding="same_zeros",
                use_bias=True,
                activation=gdn.GDN(inverse=True),
            )
            tensor = layer(tensor)
...
def train():
    ...
    with tf.device("/cpu:0"):
        ...
        print("TRAIN_GLOB : ", train_files[0])
    ...
    entropy_bottleneck = entropymod.EntropyBottleneck()
...
def compress():
    ...
    with tf.Session() as sess:
        ...
        if args.verbose:
            ...
            with open(args.output + ".txt", "w") as txtf:
...
def decompress():
    ...
    entropy_bottleneck = entropymod.EntropyBottleneck(dtype=tf.float32)
    y_hat = entropy_bottleneck.decompress(strings, y_shape, channels=args.num_filters)
    x_hat = synthesis_transform(y_hat, args.num_filters)
