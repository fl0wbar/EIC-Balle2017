...
from imagecompression.python.ops import math_ops as cmath_ops
from imagecompression.python.ops import spectral_ops as spectral_ops
...
class NonnegativeParameterizer(Parameterizer):
    ...
    def __call__(self, getter, name, shape, dtype, initializer, regularizer=None):
        ...
        bound = array_ops.constant(
            (self.minimum + self.reparam_offset**2) ** 0.5, dtype=dtype
        )
        ...
        var = getter(
            ...
            shape=shape,
            dtype=dtype,
            initializer=reparam_initializer,
            regularizer=regularizer,
        )
        return reparam(var)
