...
from imagecompression.python.layers import parameterizers
from imagecompression.python.ops import padding_ops
...
class _SignalConv(base.Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._strides_up = utils.normalize_tuple(strides_up, self._rank, "strides_up")
    ...
    def call(self, inputs):
        ...
        if (
            corr
            and all(s == 1 for s in self.strides_down)
        ...
        data_format = utils.convert_data_format(self.data_format, self._rank + 2)
        if (
            corr
            ...
            corr and all(s == 1 for s in self.strides_up) and not self.channel_separable
        ):
            ...
            and all(s == 1 for s in self.strides_down)
            and (
                (not self.channel_separable and 1 <= self._rank <= 3)
                or (
                    self.channel_separable
                    and self.filters == 1
                    and self._rank == 2
                    and all(s == self.strides_up[0] for s in self.strides_up)
                )
            )
        ):
            ...
            if self.padding == "valid":
                ...
                for i, a in enumerate(spatial_axes):
                    slices[a] = slice(
                        ...
                        None
                        if self.kernel_support[i] == 1
                        else 1 - self.kernel_support[i],
                    )
            else:
...
def _conv_class_factory(name, rank):
    ...
    clsdict = {"__init__": init, "__doc__": _SignalConv.__doc__.format(rank=rank)}
