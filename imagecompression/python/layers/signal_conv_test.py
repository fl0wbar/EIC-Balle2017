...
import imagecompression.python.layers.initializers as initializers
import imagecompression.python.layers.signal_conv as sigconv
...
class SignalTest(tf.test.TestCase):
    ...
    def run_valid(
        ...
        layer_class = {
            3: sigconv.SignalConv1D,
            4: sigconv.SignalConv2D,
            5: sigconv.SignalConv3D,
        }[inputs.ndim]
        layer = layer_class(
            filters,
            kernel_support,
    ...
    def is_implemented(
        ...
        if any(s != 1 for s in strides_down) and any(s != 1 for s in strides_up):
            return False

        # Upsampled correlations are not implemented unless kernels are odd-length.
        if (
            corr
            and any(s != 1 for s in strides_up)
            and any(s % 2 != 1 for s in kernel_support)
        ):
            return False
        ...
        if (
            not corr
            and any(s != 1 for s in strides_down)
            and any(s % 2 != 1 for s in kernel_support)
        ):
            return False
        ...
        if channel_separable and (
            len(input_support) != 2
            or any(s != strides_up[0] for s in strides_up)
            or any(s != strides_down[0] for s in strides_down)
        ):
            return False
        ...
        if (
            ...
            and (
                ...
                or (not corr and any(s % 2 != 1 for s in kernel_support))
            )
            and filters != 1
        ):
            return False
